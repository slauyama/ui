...
Target project: "Personal Material Design" (`projectId` pinned in config). First upload 2026-09-21 by explicit user choice: replaced everything EXCEPT `ui_kits/` and `templates/`, which were kept on purpose. `_ds_manifest.json` and `.thumbnail` are app-managed, left alone. A full copy of the original project lives in gitignored `input/Personal Material Design/`.
